...
- **dashboard API contract:** `WarrantSummary.proof_summary.chain_op_type` now carries Holochain 0.7's op-type names — `CreateRecord` / `CreateEntry` / `AgentActivity` / `UpdateEntry` / `UpdateRecord` / `DeleteEntry` / `DeleteRecord` / `CreateLink` / `DeleteLink`, replacing 0.6's `StoreRecord` / `StoreEntry` / `RegisterAgentActivity` / `Register*`. The field is a passthrough string, so no D1 migration is needed — but the D1 is one shared instance, so during a 0.6→0.7 window the old and new fleets' observers write it concurrently and the dashboard shows both vocabularies at once.
